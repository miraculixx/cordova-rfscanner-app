...
//  GetLocation
//
//  Created by dev on 2016. 2. 5..
//  Copyright © 2016년 com.wifi.scan. All rights reserved.
//
...
@property(nonatomic) CLLocation *hike;
@end
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations {
    ...
    if (fabs([now timeIntervalSinceDate:timestamp]) < 60){
        ...
        if (fabs([now1 timeIntervalSinceDate:timestamp]) > 15) {
            ...
            self.labelTime.text = [DateFormatter stringFromDate:timestamp];
            [self InsertData];
        }
    ...
    /*if (m_timeStamp == 15) {
        m_timeStamp = 0;
        NSLog(@"time stamp : %d", m_timeStamp);
        [self InsertData];
    }
    NSLog(@"time stamp : %f", age);*/
}
